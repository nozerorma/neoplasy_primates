...
View(overlapping_traits[,2:16])

var_quanti <- PCA(log10(overlapping_traits[,2:16]+0.01), scale.unit = TRUE)
fviz_pca_var(var_quanti, col.var="contrib",
             repel=TRUE)
var_quanti_cor <- as.data.frame(var_quanti$var$cor)
library(Hmisc)
library(corrplot)
res <- cor(log10(overlapping_traits[,2:16]+0.01),
           use="pairwise.complete.obs")
corrplot(res,
         tl.col = "black")


#First axis mainly driven by correlations 
#CLUSTERS of correlations
trans_var_quanti <- PCA(t(log10(overlapping_traits[,2:16]+0.01)), 
                        scale.unit = TRUE)

info_cluster_elbow <- fviz_nbclust(t(log10(overlapping_traits[,2:16]+0.01)), 
                                   hcut, method = "silhouette", print.summary = TRUE,
                                   )
