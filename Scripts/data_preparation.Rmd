...
# Need to name the object to work wit it (not sure why)
...
for (index in seq_along(unique_distances)) {
  ...
  tree_cluster_command <- sprintf("python3 TreeCluster/TreeCluster.py -i Out/tree_prunning/science.abn7829_data_s4.nex.tree.pruned -t %f --method root_dist -s 99 -o %s", node, output_path)
...
ordered_columns <- c("SPECIES_BINOMIAL", sort(colnames(clusters_df)[-1]))
...
```
### Visualicing cluster distribution with species present in the phylogeny and trait values

```{r, fig.width=18, fig.height=18, dev = 'png'}
...
all_levels <- unique(c(as.character(clusters_long$cluster), as.character(clusters_unique$FAMILY)))

# Plot
ggplot(clusters_long, aes(x = cut, y = SPECIES_BINOMIAL)) +
...
### Visualicing cluster distribution with species not present in the phylogeny
...
# Plot
ggplot(clusters_expanded_long, aes(x = cut, y = SPECIES_BINOMIAL)) +
  geom_tile(aes(fill = cluster), color = "white", linewidth = 0.5) +
  scale_fill_manual(values = setNames(all_colors, all_levels)) +
  scale_x_discrete(expand = c(0,0)) +
  labs(y = "Species", x = "Cut & Family", fill = "Value/Group") +
