...
heatmap_plot <- ggplot(clusters_long, aes(x = cut, y = species)) +
  ...
  labs(title = "Tree clustering by node cuts", subtitle = "Includes species outside phylogeny", caption = "Made with TreeCluster")  +
  theme_minimal() +
  theme(
    ...
    plot.title = element_text(size = 24, face = "bold", color = pokepal("rayquaza", spread = 1), hjust = 0.5, margin = margin(t = 20, b = 20)),
    plot.caption = element_text(size = 14, face = "bold", margin = margin(t = 20, b = 20)),
    plot.subtitle = element_text(size = 18, hjust = 0.5, face = "bold", margin = margin(b = 40)),
...
# Plot
heatmap_expanded_plot <- ggplot(clusters_expanded_long, aes(x = cut, y = rev(species))) +
  geom_tile(aes(fill = cluster), color = "white", linewidth = 0.5) +
  scale_fill_manual(values = setNames(all_colors, all_levels)) +
  scale_x_discrete(expand = c(0,0)) +
  labs(y = "Species", x = "Cut & Family", fill = "Value/Group") +
...
combined_plot <- quas_fam_tree + heatmap_plot
