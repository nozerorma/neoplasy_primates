...
ggplot(data = df_z, 
  ...
  geom_point(data = subset(df_z, label_family %in% c("low_extreme", "normal", "high_extreme")),
               aes(shape = label_family, color = Family, fill = Family), size = 4) +
  scale_shape_manual(values = c("low_extreme" = 22, "normal" = 21, "high_extreme" = 24)) +
  stat_summary(fun.x = median, geom = "errorbar",
        aes(xmax = after_stat(x), xmin = after_stat(x)), linewidth = 1, color = pokepal("rayquaza", spread = 1)) +
...
ggplot(data = df_z, 
  ...
  geom_point(data = subset(df_z, label_family_2sd %in% c("very_low_extreme", "normal", "very_high_extreme")),
               aes(shape = label_family_2sd, color = Family, fill = Family), size = 4) +
...
obj_np<-setMap(obj_np,pokepal("kyogre", spread = 4))
...
scatter_plot <- ggplot(data = merged_df, 
  ...
  scale_shape_manual(values = c("high_outlier_neoplasia_necropsy" = 17, "low_outlier_neoplasia_necropsy" = 22)) +
...
# Normalized scatterpot
scatter_plot_normal <- ggplot(data = merged_df, 
                      aes(x = normalize(value.x), y = normalize(value.y), color = Family)) + 
  geom_point(data = subset(merged_df, 
                      !(outlier.y %in% c("high_outlier_neoplasia_necropsy", "low_outlier_neoplasia_necropsy"))),
                      size = 4) +  
  # Plot outliers for y variable with a different shape
  ...
  # Label species that are outliers for y variable
  geom_text(data = subset(merged_df, outlier.y %in% c("high_outlier_neoplasia_necropsy", "low_outlier_neoplasia_necropsy")),
            aes(label = Species), nudge_y = 0.025, size = 4, vjust = "right", hjust = "top") +
...
write.table(extreme_table, file.path(resultsDir, "4.CAAS_analysis/config.tab"), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
